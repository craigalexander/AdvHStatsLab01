...
You **DO NOT** need to intall R or RStudio on your own laptop or desktop computer.  You are expected to complete these labs on the universities PCs, and these already have R and RStudio installed.  However, R and (basic) RStudio are free to download and install, so Lab 0 is an optional lab which has technical details on installing R and RStudio on your computer. If you already had R and/or RStudio installed, we recommend updating to the latest version before you work through this course. 
</div>

## RStudio {#rstudio_ide}

The R software gives the computer the ability to process the R programming language. We will never use that software directly. Instead, we will use [RStudio](http://www.rstudio.com){target="_blank"}, an integrated development environment (IDE) which runs R for us via a graphical user interface (GUI) .  The RStudio IDE is arranged with four window.
