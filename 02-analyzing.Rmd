...
The text file `ratings_seasons.csv` contains data on the first 10 series (2010-2019) of GBBO.   The file is on the Moodle page below the link to this lab.  When you click on it, it will download automatically to a local folder, probably the `Downloads` folder.  Find where it is saved (e.g. use the `Show in folder` option when you click on the options next to the filename at the bottom of your browser) and move it to the **same folder** as you saved (or will save) the script file with the code to analyse this data.  

Then open/go to the script file in RStudio (perhaps call it `S1Y_Lab_1.R` if you haven't already) and paste and run this code to read the data in the file `ratings_seasons.csv` into RStudio.
...
You can tell RStudio where (in what directory on your computer) to find the data set. To do this select

`> Session > Set Working Directory > Choose Directory…`

and specify the location where the file you want to read in is.
